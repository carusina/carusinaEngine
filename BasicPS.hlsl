...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    for (int i = 0; i < MAX_LIGHTS; ++i)
    {
        if (lights[i].type)
        {
            ...
            float3 centerToRay = dot(L, reflectedDir) * reflectedDir - L;
            float3 representativePoint = L + centerToRay * saturate(lights[i].radius / length(centerToRay + 0.001)); // posWorld로부터의 상대적인 위치, 0.001 => 0 나누기 방지
            representativePoint += input.posWorld; // world 기준으로 바꾸기 위해서 더해줌
            ...
            float alpha = roughness * roughness;
            float alphaPrim = saturate(alpha + lights[i].radius / (3.0 * lightDist));
            
            float D = NdfGGX(NdotH, roughness) * pow(alpha / alphaPrim, 2.0);
